                          ...
                          size_t& block_size)
{
    ...
    try {
        ...
        desc.add_options()
          ...
          ("source_file,sf", po::value<std::filesystem::path>(&source_file)->required(), "Source file")
          ("destination_file,df", po::value<std::filesystem::path>(&destination_file)->required(), "Destination file")
          ("block_size,bs", po::value<size_t>(&block_size)->default_value(job::gkDefaultBlockSize), "Hash block size")
        ;
